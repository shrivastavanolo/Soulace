    ...
    <div className="bg-gray-800 rounded-lg shadow-md p-4 mb-4 transition-all duration-300 hover:shadow-lg border-l-4 border-indigo-500">
      <div className="flex justify-between items-center">
        <h4 className="text-lg font-semibold text-gray-200 flex items-center">
          {getEmoji()} {subject.name || `Subject ${index + 1}`}
        </h4>
      ...
      {isExpanded && (
        <div className="mt-2 text-sm text-gray-300">
          ...
          {isGoodMark ? (
            <p className="text-emerald-400">🎉 Excellent work! Keep up the great performance.</p>
          ) : (
            <p className="text-amber-400">💪 There's room for improvement. With focused effort, you can boost your performance.</p>
          )}
        </div>
      )}
    </div>
...
export default function Report() {
  ...
  const generateReportCard = useCallback(async (retryCount = 0) => {
    ...
    const prompt = `Generate a detailed, encouraging, and positive report card for a student named ${studentName} in class ${studentClass}. The student's marks are as follows: ${subjectsData}. 

    Provide the following sections:
    1. Overall Assessment (7-8 sentences)
    2. Strengths (4-5 points)
    3. Areas for Improvement (4-5 points)
    4. Motivational Message (4-5 sentences)

    Focus on each subject and discuss about the performance in that subject

    Use a conversational and friendly tone. Provide detailed insights into the student's performance, going beyond just the numerical grades. The tone should be very positive, supportive, and natural-sounding.

    Important: Do not use any asterisks, stars, bullet points, or any kind of formatting in your response. Use plain text only, with each point on a new line. Include appropriate emojis at the start of each point or sentence to make the report card more lively and engaging.`
